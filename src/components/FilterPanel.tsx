  ...
  return (
    <div className="flex h-full flex-col gap-5 overflow-y-auto p-4">
      <div className="flex items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-stone-500" aria-live="polite">
            {resultCount === null ? 'Counting…' : `${resultCount.toLocaleString()} trees match`}
          </p>
        </div>
        <button
          type="button"
      ...
      <label className="block">
        <span className="field-label">Search notes and addresses</span>
        <input
          type="search"
          className="field-input"
          value={filters.q}
          placeholder="e.g. schoolyard, Mashtots"
              ...
              <div className="flex flex-wrap gap-1.5">
                {group.options.map((option) => (
                  ...
                  >
                    {option.nameEn}
                    {option.nameHy ? (
                      <span className="opacity-70">{option.nameHy}</span>
                    ) : null}
                  </button>
                ))}
              </div>
