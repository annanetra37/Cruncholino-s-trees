...
import { CONDITIONS, FRUIT_QUALITIES, conditionColor, labelFor } from '@/lib/constants';
    ...
    <div className="mx-auto max-w-4xl px-4 py-6">
      ...
      {trees.length === 0 ? (
        <p className="mt-8 rounded-xl border border-dashed border-stone-300 p-8 text-center text-stone-500">
          You haven’t recorded any trees yet.
        </p>
      ) : (
            ...
            <li key={tree.id} className="card flex flex-wrap items-center gap-3 p-3">
              ...
              <div className="min-w-40 flex-1">
                <p className="font-semibold">
                  {tree.species.nameEn}
                  {tree.species.nameHy ? (
                    <span className="ml-1 font-normal text-stone-400">{tree.species.nameHy}</span>
                  ) : null}
                </p>
                <p className="text-sm text-stone-500">
                  {tree.addressLine ?? `${tree.latitude.toFixed(4)}, ${tree.longitude.toFixed(4)}`}
                </p>
              </div>

              <p className="text-sm text-stone-600">
                {labelFor(CONDITIONS, tree.condition)} · {labelFor(FRUIT_QUALITIES, tree.fruitQuality)}
              </p>
